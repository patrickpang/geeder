...
async function createCard(payload: Card): Promise<boolean> {
  try {
...
function CardEditor({ card, onComplete }: CardEditorProps) {
  const stateSignal = useSignal<"editing" | "submitting" | "success" | "error">(
    "editing",
  );
  ...
  const onSubmit: JSX.SubmitEventHandler<HTMLFormElement> = async (e) => {
    ...
    const ok = await createCard(payload);
    stateSignal.value = ok ? "success" : "error";
    if (onComplete) onComplete();
  };

  if (stateSignal.value === "success") {
    return (
      <div role="alert" class="alert alert-success text-base-100">
